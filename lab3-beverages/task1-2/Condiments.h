...
public:
	CChocolate(IBeveragePtr&& beverage, unsigned quantity)
		: CCondimentDecorator(move(beverage))
	{
		quantity > 5 ? m_quantity = 5 : m_quantity = quantity;
	}
...
{
public:
	CLiquor(IBeveragePtr&& beverage, LiquorType liquor)
		: CCondimentDecorator(move(beverage))
		, m_liqourType(liquor)
	{
	}
...
protected:
	...
	std::string GetCondimentDescription() const override
	{
		return std::string(m_liqourType == LiquorType::Chocolate ? "Chocolate" : "Nut")
			+ " liquor";
	}

private:
	LiquorType m_liqourType;
};
