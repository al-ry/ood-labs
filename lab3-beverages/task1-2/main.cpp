...
void Test()
{
	...
	auto teaWithChocolate = make_unique<CChocolate>(move(tea), 5);
	cout << teaWithChocolate->GetDescription() << " " << teaWithChocolate->GetCost() << endl;

	auto greenTea = make_unique<CTea>(KindOfTea::Green);
	cout << greenTea->GetDescription() << " " << greenTea->GetCost() << endl;

	auto cappuccinoStandart = make_unique<CCappuccino>();
	cout << cappuccinoStandart->GetDescription() << " " << cappuccinoStandart->GetCost() << endl;

	auto cappuccinoWithChocolate = make_unique<CChocolate>(move(cappuccinoStandart), 1);
	cout << cappuccinoWithChocolate->GetDescription() << " " << cappuccinoWithChocolate->GetCost() << endl;

	auto doubleCappuccino = make_unique<CCappuccino>(CoffeePortion::Double);
	cout << doubleCappuccino->GetDescription() << " " << doubleCappuccino->GetCost() << endl;

	auto doubleCappuccinoWithCream = make_unique<CCream>(move(doubleCappuccino));
	cout << doubleCappuccinoWithCream->GetDescription() << " " << doubleCappuccinoWithCream->GetCost() << endl;

	auto milkshake = make_unique<CMilkshake>(MilkshakePortion::Medium);
	cout << milkshake->GetDescription() << " " << milkshake->GetCost() << endl;
	auto milkshakeWithChocolateLiquor = make_unique<CLiquor>(move(milkshake), LiquorType::Chocolate);
	cout << milkshakeWithChocolateLiquor->GetDescription() << " " << milkshakeWithChocolateLiquor->GetCost() << endl;

	auto smallMilkshake = make_unique<CMilkshake>(MilkshakePortion::Small);
	cout << smallMilkshake->GetDescription() << " " << smallMilkshake->GetCost() << endl;

	auto smallMilkshakeWithNutLiquor = make_unique<CLiquor>(move(smallMilkshake), LiquorType::Nut);
	cout << smallMilkshakeWithNutLiquor->GetDescription() << " " << smallMilkshakeWithNutLiquor->GetCost() << endl;

	auto largeMilkshake = make_unique<CMilkshake>(MilkshakePortion::Large);
	cout << largeMilkshake->GetDescription() << " " << largeMilkshake->GetCost() << endl;
}
...
int main()
{
	...
	//cout << endl;
	{
		...
		cout << latte->GetDescription() << endl;
		// добавляем корицы
